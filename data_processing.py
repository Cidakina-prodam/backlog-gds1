...
# Ordem usada no histórico por status e nos gráficos (inclui a categoria "Suspensa",
# que não é um status real do GDP, e sim o campo 'status' já sobrescrito no record
# quando Suspensao == 'Sim' — ver build_dataset).
STATUS_HISTORICO_ORDER = OPEN_STATUS + ['Suspensa']

# Definição de "backlog aberto" (set/2026): só o que está EM ANDAMENTO.
# Demandas em homologação (já entregues, com o cliente) e suspensas continuam
# no recorte, mas são contadas à parte.
STATUS_ANDAMENTO = ['Aberta', 'Planejamento', 'Aprovar Planej.', 'Planej. Aprovado', 'Execução']
STATUS_HOMOLOGACAO = ['Homologação', 'Homolog. Expressa', 'Homolog. Tácita']


def grupo_de(record):
    """'andamento' | 'homologacao' | 'suspensa'."""
    if record.get('suspensa'):
        return 'suspensa'
    if record.get('status') in STATUS_HOMOLOGACAO:
        return 'homologacao'
    return 'andamento'

# Status terminais: a demanda saiu do backlog em definitivo. Não entram em
# `records` (que é o backlog), mas alimentam a leitura de vazão (o que saiu
# na semana), usando DataStatus como a data em que a saída aconteceu.
CLOSED_STATUS = ['Concluída', 'Cancelada']

# Janela da leitura de vazão: 7 dias contados para trás a partir da data da coleta.
JANELA_VAZAO_DIAS = 7

# Demandas criadas antes deste ano contam como passivo herdado; do ano em diante,
# como demanda corrente. Derivado da data da coleta, então o corte acompanha o
# ano em que o painel está rodando.
def ano_corte(today):
    return today.year
...
def norm_sim_nao(v):
    """Normaliza campo Sim/Não do GDP (ex.: Suspensao) para bool."""
    if v is None:
        return False
    return str(v).strip().lower() == 'sim'


def norm_texto_opcional(v):
    """Normaliza texto que pode vir vazio ou como '-' (padrão do export quando não se aplica)."""
...
def _norm_col(nome):
    """Nome de coluna comparável: sem acento, maiúsculo, só letras e números."""
    import unicodedata
...
def get_ngds(rd):
    """
    Lê o ID do NGDS da linha do export (coluna 'ID_NGDS'). Tolera diferenças
    de grafia no cabeçalho (espaço sobrando, maiúsculas, 'ID NGDS'), mas só
    aceita coluna que seja de fato o ID — nunca 'HorasExecutadasNgds', que
    também tem NGDS no nome.
    """
    v = rd.get('ID_NGDS')
    if v not in (None, ''):
        return v
    for k, val in rd.items():
        if k and _norm_col(k) == 'IDNGDS' and val not in (None, ''):
            return val
    return None
...
def build_dataset(uploaded_files, nucleo_mapping, today=None):
    ...
    janela_fim = today.date() if hasattr(today, 'date') else today
    janela_ini = janela_fim - datetime.timedelta(days=JANELA_VAZAO_DIAS)

    def _na_janela(d):
        return d is not None and janela_ini < d <= janela_fim

    for rd in all_rows:
        status = rd.get('Status')
        if status not in OPEN_STATUS and status not in CLOSED_STATUS:
            continue
        ...
        esf_fat = norm_num(rd.get('EsforcoFaturado'))

        # --- Suspensão: sinalizada pelo GDP em ['Suspensao','DtInicioSuspensao',
        #     'DtFimSuspensao','MotivoSuspensao']. Uma demanda suspensa continua
        #     no recorte de backlog aberto (ainda não foi concluída), mas:
        #     - não conta no KPI "Backlog aberto" (fica como categoria à parte)
        #     - aparece na tabela/funil com status "Suspensa" (status real vai
        #       para statusReal, como info extra)
        #     - o aging (diasAberto) congela na data de início da suspensão,
        #       em vez de continuar contando até hoje
        status_real = status
        suspensa = norm_sim_nao(rd.get('Suspensao'))
        dt_inicio_susp = norm_date(rd.get('DtInicioSuspensao'))
        dt_fim_susp = norm_date(rd.get('DtFimSuspensao'))
        motivo_susp = norm_texto_opcional(rd.get('MotivoSuspensao'))
        status = 'Suspensa' if suspensa else status_real

        if suspensa and dt_inicio_susp:
            dias = (dt_inicio_susp - dc).days if dc else None
        else:
        ...
        dt_status = norm_date(rd.get('DataStatus'))
        encerrada = status_real in CLOSED_STATUS
        # Passivo x corrente pela data de CRIAÇÃO da demanda.
        bloco = 'corrente' if (dc and dc.year >= ano_corte(today)) else 'passivo'

        # --- Vazão: o que se moveu dentro da janela de 7 dias. Uma mesma demanda
        #     pode aparecer como entrada e como saída (criada e concluída na mesma
        #     semana), por isso os eventos são registrados em campos separados.
        ev_entrada = _na_janela(dc.date() if hasattr(dc, 'date') else dc)
        ev_encerrou = encerrada and _na_janela(dt_status.date() if hasattr(dt_status, 'date') else dt_status)
        ev_suspendeu = suspensa and _na_janela(
            dt_inicio_susp.date() if hasattr(dt_inicio_susp, 'date') else dt_inicio_susp)
        # Movimento interno: mudou de status na janela sem ser encerramento.
        ev_mudou = (not encerrada) and _na_janela(
            dt_status.date() if hasattr(dt_status, 'date') else dt_status)

        if ev_entrada or ev_encerrou or ev_suspendeu or ev_mudou:
            movimentos.append({
                ...
                'bloco': bloco,
                'status': status,
                'statusReal': status_real,
                'entrada': ev_entrada,
                'encerrou': ev_encerrou,
                'suspendeu': ev_suspendeu,
                'mudou': ev_mudou,
                'dataCriacao': dc.strftime('%d/%m/%Y') if dc else None,
                'dataStatus': dt_status.strftime('%d/%m/%Y') if dt_status else None,
            })
        ...
        records.append({
            'bloco': bloco,
            'id': norm_id(rd.get('ID_GDP')),
            'idNgds': norm_id(get_ngds(rd)) or '',
            'titulo': (rd.get('TituloDemanda') or '').strip(),
            'status': status,
            'statusReal': status_real,
            'suspensa': suspensa,
            'motivoSuspensao': motivo_susp,
            'dtInicioSuspensao': dt_inicio_susp.strftime('%d/%m/%Y') if dt_inicio_susp else None,
            'dtFimSuspensao': dt_fim_susp.strftime('%d/%m/%Y') if dt_fim_susp else None,
            'nucleoNegocio': nuc_neg,
    ...
    if records and not any(r['idNgds'] for r in records):
        cols_ngds = sorted({k for rd in all_rows for k in rd.keys() if k and _norm_col(k) == 'IDNGDS'})
        if cols_ngds:
            avisos.append(
                "Nenhuma demanda veio com NGDS preenchido. Coluna(s) de NGDS encontrada(s) no export: "
                + ", ".join(cols_ngds) + " — todas estão vazias nas demandas em aberto."
            )
        else:
            avisos.append(
                "O export não trouxe nenhuma coluna de NGDS (procurei pela coluna ID_NGDS). "
                "Para a coluna NGDS aparecer no painel, ela precisa vir na exportação do GDP."
            )
    ...
    dataset = {
        ...
        'janelaVazao': {
            'inicio': janela_ini.strftime('%d/%m/%Y'),
            'fim': janela_fim.strftime('%d/%m/%Y'),
            'dias': JANELA_VAZAO_DIAS,
        },
        'anoCorte': ano_corte(today),
...
def snapshot_row(records, nucleo, snapshot_date):
    no_escopo = [r for r in records if nucleo == 'GDS-1' or r['nucleoNegocio'] == nucleo]
    # backlog aberto = em andamento (sem homologações e sem suspensas)
    recs = [r for r in no_escopo if grupo_de(r) == 'andamento']
    homolog = sum(1 for r in no_escopo if grupo_de(r) == 'homologacao')
    suspensas = sum(1 for r in no_escopo if grupo_de(r) == 'suspensa')
    total = len(recs)
    legado = sum(1 for r in recs if r.get('bloco') == 'passivo')
...
# ---------------------------------------------------------------------------
# Histórico por status (série usada na seção "Evolução por status")
# ---------------------------------------------------------------------------

def build_status_history_rows(records, snapshot_date):
    """
    Uma linha por (núcleo, status) nessa coleta — inclui 'GDS-1' como o
    agregado geral (todos os núcleos) e 'Suspensa' como mais uma categoria de
    status, ao lado dos status reais abertos. Núcleos/status sem nenhuma
    demanda na coleta entram com count=0 (mantém a série contínua p/ gráfico).
    """
    nucleos_presentes = sorted(set(r['nucleoNegocio'] for r in records) - {'Não classificado'})
    escopos = nucleos_presentes + ['GDS-1']
    rows = []
    for nuc in escopos:
        recs = [r for r in records if nuc == 'GDS-1' or r['nucleoNegocio'] == nuc]
        counts = Counter(r['status'] for r in recs)
        for status in STATUS_HISTORICO_ORDER:
            rows.append({
                'data_snapshot': snapshot_date,
                'nucleo': nuc,
                'status': status,
...
def append_status_history(history_rows_existing, new_rows, replace_same_date=True):
    """Mesma lógica de append_history, só que a chave inclui o status."""
    key = lambda r: (r['data_snapshot'], r['nucleo'], r['status'])
    existing = {key(r): r for r in history_rows_existing} if replace_same_date else {}
    for r in new_rows:
        existing[key(r)] = r
    out = list(existing.values())
    out.sort(key=lambda r: (r['data_snapshot'], r['nucleo'], r['status']))
    return out


STATUS_HISTORY_FIELDS = ['data_snapshot', 'nucleo', 'status', 'count']


def status_history_to_csv_bytes(status_history_rows):
    buf = io.StringIO()
    w = csv.DictWriter(buf, fieldnames=STATUS_HISTORY_FIELDS, delimiter=';')
    w.writeheader()
    w.writerows(status_history_rows)
    return buf.getvalue().encode('utf-8')


def status_history_from_csv_bytes(file_bytes):
...
HISTORY_FIELDS = ['data_snapshot', 'nucleo', 'backlog_total', 'sem_prazo_count',
                   ...
                   'backlog_legado', 'backlog_atual', 'backlog_homologacao', 'suspensas']


def history_to_csv_bytes(history_rows):
    buf = io.StringIO()
    w = csv.DictWriter(buf, fieldnames=HISTORY_FIELDS, delimiter=';', restval='', extrasaction='ignore')
    w.writeheader()
...
def history_from_csv_bytes(file_bytes):
    ...
    for r in reader:
        ...
        # legado/atual só existem em coletas processadas a partir do painel novo;
        # nas mais antigas ficam em branco (o gráfico mostra só o total nessas semanas).
        # backlog_homologacao/suspensas só existem a partir da regra "em andamento";
        # linhas sem esses campos são da regra antiga (que contava homologação junto).
        for k in ('backlog_legado', 'backlog_atual', 'backlog_homologacao', 'suspensas'):
            v = r.get(k)
            if v in (None, ''):
                r.pop(k, None)
            else:
...
def extract_state_from_html(html_bytes):
    """
    Extrai o histórico semanal, o histórico por status e o mapeamento
    sigla->núcleo de dentro de um HTML já distribuído anteriormente (o
    próprio dashboard.html carrega tudo embutido em `const DATA = {...}`).
    Útil como alternativa simples ao backup/restore em .zip: quem já guarda
    os HTMLs distribuídos por e-mail não precisa de mais nada pra recuperar
    o histórico.

    Retorna (history_rows, mapping, records, snapshot_date, status_history_rows, avisos).
    """
    text = html_bytes.decode('utf-8', errors='replace')
    marker = 'const DATA = '
    idx = text.find(marker)
    if idx == -1:
        return [], {}, [], None, [], ["Não encontrei os dados embutidos nesse HTML — é mesmo um dashboard gerado por este app?"]
    start = idx + len(marker)
    try:
        data, _ = json.JSONDecoder().raw_decode(text, start)
    except json.JSONDecodeError as e:
        return [], {}, [], None, [], [f"Não consegui ler os dados desse HTML (arquivo corrompido ou incompleto?): {e}"]

    history_rows = data.get('history', [])
    status_history_rows = data.get('statusHistory', [])
    ...
    if not status_history_rows:
        avisos.append(
            "Esse HTML não tinha o histórico por status embutido (é de antes dessa funcionalidade existir) — "
            "a série de evolução por status vai começar a partir da próxima coleta processada, não desta restauração."
        )
    if not records:
        avisos.append(
    ...
    return history_rows, mapping, records, snapshot_date, status_history_rows, avisos
...
def _bloco_de(record, ano):
    """Legado ('passivo') x atual ('corrente') pela data de criação, sempre com o
    ano de corte da coleta ATUAL — assim, na virada do ano, a mesma demanda é
    classificada igual dos dois lados da comparação."""
    dc = _parse_br_date(record.get('dataCriacao'))
    if dc is None:
        return record.get('bloco') or 'passivo'
    return 'corrente' if dc.year >= ano else 'passivo'


def compute_bloco_flow(prev_records, curr_records, curr_date=None):
    """
    Entradas e saídas do backlog aberto entre duas coletas, separadas em legado
    (passivo) e atuais (corrente), por núcleo e para o GDS-1 inteiro.

    Usa a mesma regra do "Backlog aberto": só demandas EM ANDAMENTO. Assim:
      - ir para homologação conta como saída (contada à parte em 'paraHomologacao');
      - ser suspensa conta como saída (à parte em 'suspensas');
      - voltar da homologação conta como entrada (à parte em 'voltaramHomologacao').
    anterior + entraram - saíram = atual sempre fecha e bate com o histórico.
    """
    curr_dt = _parse_iso_date(curr_date) or datetime.date.today()
    ano = curr_dt.year
    prev = {norm_id(r.get('id')): r for r in prev_records if grupo_de(r) == 'andamento'}
    curr = {norm_id(r.get('id')): r for r in curr_records if grupo_de(r) == 'andamento'}
    suspensas_agora = {norm_id(r.get('id')) for r in curr_records if grupo_de(r) == 'suspensa'}
    homolog_agora = {norm_id(r.get('id')) for r in curr_records if grupo_de(r) == 'homologacao'}
    homolog_antes = {norm_id(r.get('id')) for r in prev_records if grupo_de(r) == 'homologacao'}

    ORDEM_NUCLEOS = ['NSS1', 'NSS2', 'NSS3', 'NC']
    presentes = ({r.get('nucleoNegocio') for r in prev.values()} |
                 {r.get('nucleoNegocio') for r in curr.values()}) - {'Não classificado', None}
    escopos = [n for n in ORDEM_NUCLEOS if n in presentes] + sorted(presentes - set(ORDEM_NUCLEOS)) + ['GDS-1']

    out = {}
    for esc in escopos:
        out[esc] = {}
        for bloco in ('passivo', 'corrente'):
            def ids(base):
                return {i for i, r in base.items()
                        if (esc == 'GDS-1' or r.get('nucleoNegocio') == esc) and _bloco_de(r, ano) == bloco}
            p_ids, c_ids = ids(prev), ids(curr)
            sairam = p_ids - c_ids
            entraram = c_ids - p_ids
            out[esc][bloco] = {
                'anterior': len(p_ids),
                'atual': len(c_ids),
                'entraram': len(entraram),
                'sairam': len(sairam),
                'paraHomologacao': len(sairam & homolog_agora),
                'suspensas': len(sairam & suspensas_agora),
                'voltaramHomologacao': len(entraram & homolog_antes),
            }
    return out
...
def compute_weekly_flow(prev_records, curr_records, prev_date=None, curr_date=None):
    ...
    return {
        'prevDate': prev_date,
        'currDate': curr_date,
        'porBloco': compute_bloco_flow(prev_records, curr_records, curr_date),
        'totals': totals,
