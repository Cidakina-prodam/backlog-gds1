...
def compute_weekly_flow(prev_records, curr_records, prev_date=None, curr_date=None):
    ...
    Compara duas coletas item a item (por ID_GDP) e classifica cada demanda em:
      - nova         : só existe na coleta atual
    ...
    """
    prev_by_id = {r.get('id'): r for r in prev_records}
    curr_by_id = {r.get('id'): r for r in curr_records}
    prev_ids = set(prev_by_id)
    curr_ids = set(curr_by_id)

    novas_ids = curr_ids - prev_ids
    ...
    for nuc in nucleos:
        ...
        mud_n = sum(1 for i in comuns_n if prev_by_id[i].get('status') != curr_by_id[i].get('status'))
        por_nucleo.append({
            'nucleo': nuc,
            'novas': len(c_ids_n - p_ids_n),
            'saidas': len(p_ids_n - c_ids_n),
            'mudancaStatus': mud_n,
            'totalAnterior': len(p_ids_n),
            'totalAtual': len(c_ids_n),
            'saldoLiquido': len(c_ids_n) - len(p_ids_n),
        })
    por_nucleo.sort(key=lambda r: -r['totalAtual'])
