...
def snapshot_row(records, nucleo, snapshot_date):
    recs = [
        r for r in records
        if (nucleo == 'GDS-1' or r['nucleoNegocio'] == nucleo) and not r.get('suspensa')
    ]
    total = len(recs)
...
def history_from_csv_bytes(file_bytes):
    ...
    for r in reader:
        ...
        for k in ('backlog_legado', 'backlog_atual'):
            v = r.get(k)
            if v in (None, ''):
                r.pop(k, None)
            else:
...
def compute_bloco_flow(prev_records, curr_records, curr_date=None):
    ...
    Usa a mesma regra do KPI "Backlog aberto": demandas suspensas ficam de fora.
    Por isso uma demanda suspensa na semana conta como saída (e é contada à
    parte em 'suspensas'), e uma que voltou da suspensão conta como entrada.
    Assim, anterior + entraram - saíram = atual sempre fecha e bate com o
    histórico semanal.
    """
    curr_dt = _parse_iso_date(curr_date) or datetime.date.today()
    ano = curr_dt.year
    prev = {norm_id(r.get('id')): r for r in prev_records if not r.get('suspensa')}
    curr = {norm_id(r.get('id')): r for r in curr_records if not r.get('suspensa')}
    suspensas_agora = {norm_id(r.get('id')) for r in curr_records if r.get('suspensa')}
