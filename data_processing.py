...
def build_dataset(uploaded_files, nucleo_mapping, today=None):
    ...
    #     mesmo que venha em arquivos diferentes ou repetidos) ---
    seen_ids = {}
    duplicados = []
    all_rows_dedup = []
    for rd in all_rows:
        _id = rd.get('ID_GDP')
        if _id in seen_ids:
            duplicados.append((_id, seen_ids[_id], rd.get('__arquivo_origem__')))
            continue
        seen_ids[_id] = rd.get('__arquivo_origem__')
        all_rows_dedup.append(rd)
    all_rows = all_rows_dedup
