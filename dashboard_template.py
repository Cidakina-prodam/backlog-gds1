...
.gauge-caption{text-align:center;margin-top:2px;font-family:var(--font-mono);font-size:11px;color:var(--text-mute);letter-spacing:.06em;}
.gauge-legend{display:flex;justify-content:center;gap:16px;margin-top:14px;font-size:11px;color:var(--text-mute);font-family:var(--font-mono);}
...
.kpi-label{font-size:11px;color:var(--text-mute);text-transform:uppercase;letter-spacing:.08em;font-family:var(--font-mono);margin-bottom:10px;}
...
.block-num{font-family:var(--font-mono);font-size:11px;color:var(--text-mute);letter-spacing:.1em;}
...
.risk-meta{font-family:var(--font-mono);font-size:10.5px;color:var(--text-mute);margin-top:4px;}
...
.flow-card-label{font-size:11px;color:var(--text-mute);text-transform:uppercase;letter-spacing:.08em;font-family:var(--font-mono);margin-bottom:8px;}
...
.flow-card-sub{font-size:11px;color:var(--text-mute);margin-top:4px;}
...
.flow-nucleo-table th{text-align:left;padding:8px 10px;color:var(--text-mute);font-family:var(--font-mono);font-size:10.5px;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid var(--border);}
...
.filter-bar .fb-label{font-family:var(--font-mono);font-size:11px;color:var(--text-mute);text-transform:uppercase;letter-spacing:.08em;display:flex;align-items:center;gap:6px;}
.filter-bar select{
...
.effort-head{display:grid;grid-template-columns:1fr 110px 110px 90px;gap:12px;font-family:var(--font-mono);font-size:10.5px;color:var(--text-mute);text-transform:uppercase;letter-spacing:.06em;padding-bottom:10px;border-bottom:1px solid var(--border);margin-bottom:2px;}
...
table{width:100%;border-collapse:collapse;font-size:12.5px;min-width:1150px;}
thead th{
  text-align:left;padding:12px 14px;background:var(--bg-raised);color:var(--text-mute);
  font-family:var(--font-mono);font-size:10.5px;text-transform:uppercase;letter-spacing:.05em;
  cursor:pointer;white-space:nowrap;border-bottom:1px solid var(--border);position:sticky;top:0;
  user-select:none;
}
...
tbody td{padding:11px 14px;border-bottom:1px solid var(--border-soft);color:var(--text-dim);white-space:nowrap;}
tbody tr:last-child td{border-bottom:none;}
tbody tr:hover{background:rgba(255,255,255,0.02);}
td.titulo{white-space:normal;max-width:280px;color:var(--text);font-size:12.5px;}
td.mono{font-family:var(--font-mono);color:var(--text-dim);}
.badge{display:inline-block;padding:3px 9px;border-radius:20px;font-size:10.5px;font-family:var(--font-mono);font-weight:600;letter-spacing:.02em;}
...
<div class="wrap">
  ...
  <header class="top">
    <div>
      <div class="top-id"><span class="dot"></span>NÚCLEO NSS1 · MÚLTIPLOS PROJETOS</div>
      <h1>Backlog GDS-1</h1>
  ...
  <section class="block">
    ...
    <div class="panel" id="flowPanel"></div>
  </section>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">03 · RESPONSABILIZAÇÃO</div>
...
const status_order=['Aberta','Planejamento','Aprovar Planej.','Execução','Homologação'];
...
function renderWeeklyFlow(){
  ...
  if(!wf){
    el.innerHTML = `<div class="flow-empty"><b>Sem dado de comparação nesta coleta.</b><br>Restaure o HTML da coleta anterior (passo 0 no app) antes de processar, para habilitar o comparativo semana a semana aqui.</div>`;
  ...
  document.getElementById('flowNote').textContent =
    `De ${fmtDateBR(wf.prevDate)} para ${fmtDateBR(wf.currDate)} — ${fmt(t.totalAnterior)} → ${fmt(t.totalAtual)} demandas em aberto.`;
  ...
  let html = `
    <div class="flow-cards">
      <div class="flow-card novas">
        ...
        <div class="flow-card-sub">entraram no backlog na semana</div>
      </div>
  ...
  if(wf.porNucleo && wf.porNucleo.length){
    html += `<div class="flow-subtitle">Por núcleo</div>
    <table class="flow-nucleo-table">
      <thead><tr><th>Núcleo</th><th>Novas</th><th>Saíram</th><th>Mudaram status</th><th>Anterior → Atual</th><th>Saldo</th></tr></thead>
      <tbody>
      ${wf.porNucleo.map(n=>{
        const cls = n.saldoLiquido>0 ? 'saldo-pos' : (n.saldoLiquido<0 ? 'saldo-neg' : 'saldo-zero');
        return `<tr>
          ...
          <td class="${cls}">${n.saldoLiquido>0?'+':''}${fmt(n.saldoLiquido)}</td>
        </tr>`;
      }).join('')}
      </tbody>
    </table>`;
  ...
  html += `<div class="flow-subtitle">Transições de status</div>`;
  if(wf.transicoes && wf.transicoes.length){
    const maxT = Math.max(...wf.transicoes.map(x=>x.count));
    html += wf.transicoes.slice(0,12).map(x=>`
      <div class="flow-trans-row">
        <div class="flow-trans-lbl">${x.de} <span style="color:var(--text-mute)">→</span> <b>${x.para}</b></div>
        <div class="flow-trans-track"><div class="flow-trans-fill" style="width:${(x.count/maxT*100).toFixed(1)}%"></div></div>
        <div class="flow-trans-count">${x.count}</div>
      </div>`).join('');
  } else {
    html += `<div class="risk-meta">Nenhuma demanda mudou de status entre as duas coletas.</div>`;
  }
...
function initFilters(){
  ...
  ['searchInput','filterStatus','filterSemPrazo'].forEach(id=>{
