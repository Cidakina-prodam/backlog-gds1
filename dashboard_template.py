...
.dlg-scroll thead th{cursor:default}
...
.dlist .table-controls{display:flex;gap:10px;margin-bottom:14px;flex-wrap:wrap;align-items:center}
.dlist .table-controls input[type=text],.dlist .table-controls select{border:1px solid var(--line);background:#fff;color:var(--ink);padding:9px 12px;border-radius:8px;font:inherit;font-size:14px;min-height:40px}
.dlist .table-controls input[type=text]{min-width:220px}
.dlist .table-controls select{max-width:340px}
.dlist .chk-wrap{display:flex;align-items:center;gap:7px;font-size:14px;color:var(--muted);margin-left:auto;cursor:pointer;user-select:none}
.dlist .table-count-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;flex-wrap:wrap}
.dlist .table-count{font-size:14px;color:var(--muted)}
.dlist .table-scroll{overflow-x:auto;border:1px solid var(--line);border-radius:12px}
.dlist table{width:100%;border-collapse:collapse;font-size:13px;table-layout:fixed;min-width:980px}
.dlist thead th{text-align:left;padding:10px 8px;background:#f3f5f8;color:var(--muted);font-size:12.5px;font-weight:500;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border-bottom:1px solid var(--line);position:sticky;top:0;user-select:none}
.dlist thead th .arrow{opacity:0;margin-left:2px}
.dlist thead th.sorted .arrow{opacity:1;color:var(--NSS1)}
.dlist tbody td{padding:9px 8px;border-bottom:1px solid var(--line-soft);color:#3a4556;white-space:normal;word-break:break-word;overflow:hidden}
.dlist tbody tr:hover{background:#f7f9fb}
.dlist td.titulo{color:var(--ink)}
.dlist td.trunc{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dlist th:nth-child(1),.dlist td:nth-child(1){width:8%;white-space:nowrap}
.dlist th:nth-child(2),.dlist td:nth-child(2){width:6%}
.dlist th:nth-child(3),.dlist td:nth-child(3){width:22%}
.dlist th:nth-child(4),.dlist td:nth-child(4){width:6%}
.dlist th:nth-child(5),.dlist td:nth-child(5){width:7%}
.dlist th:nth-child(6),.dlist td:nth-child(6){width:11%}
.dlist th:nth-child(7),.dlist td:nth-child(7){width:9%}
.dlist th:nth-child(8),.dlist td:nth-child(8){width:9%}
.dlist th:nth-child(9),.dlist td:nth-child(9){width:5%;white-space:nowrap}
.dlist th:nth-child(10),.dlist td:nth-child(10){width:6%}
.dlist th:nth-child(11),.dlist td:nth-child(11){width:5%}
.dlist th:nth-child(12),.dlist td:nth-child(12){width:6%}
.dlist .status-pill{white-space:nowrap;max-width:100%;overflow:hidden;text-overflow:ellipsis;vertical-align:middle}
.dlist .badge{max-width:100%;overflow:hidden;text-overflow:ellipsis;padding:3px 7px}
.dlist td:nth-child(n+10),.dlist th:nth-child(n+10),.dlist td:nth-child(9),.dlist th:nth-child(9){text-align:right}
...
.sec-title{margin:0;font-size:21px;font-weight:600}
.legend-sub{margin:6px 0 12px;font-size:13.5px}
.vnum{display:flex;flex-direction:column;align-items:flex-start}
.vlab{font-size:14px;color:var(--muted);margin-top:4px}
.extra-line{margin-top:12px}
.lnk{background:none;border:0;padding:0;color:var(--NSS1);text-decoration:underline;text-decoration-color:#c5d5f0;text-underline-offset:3px;cursor:pointer;font-size:inherit}
.lnk:hover{text-decoration-color:currentColor}
.stgrp{display:grid;grid-template-columns:190px 1fr 60px 64px;align-items:center;gap:14px;width:100%;background:#f3f5f8;border:0;border-radius:8px;padding:9px 8px;margin-top:10px;font-size:15px;font-weight:600;color:inherit;cursor:pointer;text-align:left}
.stgrp:first-child{margin-top:0}
.stgrp:hover{background:#e9edf2}
.stgrp .nm small{display:block;font-weight:400;font-size:13px;color:var(--muted)}
.stgrp .n,.stgrp .dl{text-align:right}
@media (max-width:640px){.stgrp{grid-template-columns:130px 1fr 44px 52px;font-size:14px}}
table.mtx tr.grp td{background:#f3f5f8;font-weight:700}
table.mtx tr.grp td:first-child small{font-weight:400;color:var(--muted);font-size:13px}
table.mtx tr.g-suspensa td{color:var(--muted)}
table.mtx td.ind{padding-left:28px}
.aging{display:flex;gap:16px;align-items:flex-end;min-height:230px;padding-top:8px}
.agcol{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:6px;background:none;border:0;border-radius:10px;padding:8px 4px;cursor:pointer;color:inherit}
.agcol:hover{background:#f3f6fa}
.agn{font-family:var(--font-display);font-size:20px;font-weight:700}
.agbar{width:70%;max-width:120px;border-radius:6px 6px 0 0;display:block}
.aglab{font-size:14px;font-weight:600;margin-top:4px}
.agpct{font-size:12.5px;color:var(--muted)}
...
<div class="wrap">
  ...
  <header class="top">
    ...
    <div class="top-meta">
      ...
      <div><b id="metaTotalBacklog">—</b> em andamento (backlog aberto)</div>
    </div>
  </header>
  ...
  <section class="split">
    <div class="panel card">
      ...
      <div class="note extra-line" id="legExtra"></div>
    ...
    <div class="panel card">
      ...
      <div class="note extra-line" id="atuExtra"></div>
      <div class="bar-row" id="atuBar"></div>
    </div>
  </section>

  <section class="panel">
    <h2 id="chartTitle" class="sec-title">Evolução semanal do backlog em andamento</h2>
    <div class="legend legend-sub" id="chartLegend"></div>
  ...
  <section class="panel">
    <div class="panel-head">
      <h2 id="agTitle">Aging do backlog em andamento</h2>
    </div>
    <div class="note" style="margin-bottom:8px">Dias corridos desde a criação da demanda até a data da coleta. Homologações e suspensas não entram.</div>
    <div class="aging" id="aging"></div>
    <div class="note" id="agNote"></div>
  </section>

  <section class="panel">
    <h2 id="stTitle" class="sec-title">Por status</h2>
    <div class="legend legend-sub">Barras:
  ...
  <!-- LISTAGEM (mantida) -->
  <section class="panel dlist" id="listagem">
    <div class="panel-head">
    ...
    <div class="table-scroll">
      <table>
        <thead>
          <tr>
            <th data-key="id">GDP<span class="arrow">▾</span></th>
            ...
            <th data-key="esfEst" title="Esforço estimado, em horas">Estimado<span class="arrow">▾</span></th>
            <th data-key="esfReal" title="Esforço realizado (horas lançadas), em horas">Real<span class="arrow">▾</span></th>
            <th data-key="esfFat" title="Esforço faturado, em horas">Faturado<span class="arrow">▾</span></th>
          </tr>
        </thead>
  ...
  <footer class="foot">Backlog aberto = demandas em andamento (Aberta, Planejamento, Aprovar Planej., Planej. Aprovado e Execução). Demandas em homologação (inclusive expressa e tácita) e suspensas aparecem à parte e não contam no backlog aberto.</footer>
</div>
...
<dialog id="dlg" aria-labelledby="dlgTitle">
  ...
  <div class="dlist">
    <div class="table-scroll dlg-scroll">
      <table>
        <thead><tr><th>GDP</th><th>NGDS</th><th>Título</th><th>Núcleo</th><th>Projeto</th><th>Status</th><th>Gerência (Prodam)</th><th>Gestor</th><th>Dias</th><th title="Esforço estimado, em horas">Estimado</th><th title="Esforço realizado (horas lançadas), em horas">Real</th><th title="Esforço faturado, em horas">Faturado</th></tr></thead>
...
// ---------- Grupos (definição de backlog aberto) ----------
const ANDAMENTO = ['Aberta','Planejamento','Aprovar Planej.','Planej. Aprovado','Execução'];
const HOMOLOG = ['Homologação','Homolog. Expressa','Homolog. Tácita'];
const grupoOf = r => r.suspensa ? 'suspensa' : (HOMOLOG.includes(r.status) ? 'homologacao' : 'andamento');
const GRUPO_NOME = {andamento:'Em andamento', homologacao:'Em homologação', suspensa:'Suspensas'};
const andamentoRecs = k => scopeRecs(k).filter(r=>grupoOf(r)==='andamento');

// ---------- Histórico ----------
const numOrNull = v => (v===undefined||v===''||v===null) ? null : +v;
function histOf(k){
  return HISTORY.filter(h=>h.nucleo===k)
    .map(h=>({
      date:h.data_snapshot,
      total:+h.backlog_total,
      leg:numOrNull(h.backlog_legado),
      atu:numOrNull(h.backlog_atual),
      hom:numOrNull(h.backlog_homologacao),
      nova:numOrNull(h.backlog_homologacao)!==null,   // gravada já com a regra "em andamento"
    }))
    .sort((a,b)=>a.date<b.date?-1:1);
}
// só compara semanas gravadas com a mesma regra
function comparable(h){
  const L = h.length-1; if(L<0) return [];
  const out=[h[L]];
  for(let i=L-1;i>=0 && h[i].nova===h[L].nova;i--) out.unshift(h[i]);
  return out;
}
...
// link que abre a lista de um grupo (homologação / suspensas)
function extraLinks(k, bloco){
  const base = scopeRecs(k).filter(r=>!bloco || (bloco==='passivo')===isLeg(r));
  const nh = base.filter(r=>grupoOf(r)==='homologacao').length, ns = base.filter(r=>grupoOf(r)==='suspensa').length;
  const b = bloco ? `data-bloco="${bloco}"` : '';
  return `<span class="extra">+ <button type="button" class="lnk" data-g="homologacao" ${b}>${fmt(nh)} em homologação</button>`+
         ` · <button type="button" class="lnk" data-g="suspensa" ${b}>${fmt(ns)} suspensa${ns===1?'':'s'}</button></span>`;
}
function wireLinks(el){
  el.querySelectorAll('.lnk').forEach(b=>b.onclick=()=>openGroup(cur, b.dataset.g, b.dataset.bloco||null));
}

// ---------- Veredito ----------
function renderVerdict(){
  const hAll = histOf(cur), h = comparable(hAll);
  const total = andamentoRecs(cur).length;
  const L = h.length-1;
  ...
  let t1;
  if(delta===null) t1 = hAll.length>1
    ? 'Primeira coleta com a regra "em andamento": a comparação aparece a partir da próxima.'
    : 'Primeira coleta registrada: a comparação aparece a partir da próxima.';
  else {
  ...
  if(fl && fa){
    const inn = fl.entraram + fa.entraram, out = fl.sairam + fa.sairam;
    const ph = (fl.paraHomologacao||0)+(fa.paraHomologacao||0), sus = fl.suspensas + fa.suspensas;
    const vh = (fl.voltaramHomologacao||0)+(fa.voltaramHomologacao||0);
    const det = [ph?`${fmt(ph)} para homologação`:'', sus?`${fmt(sus)} suspensas`:''].filter(Boolean).join(', ');
    t2 = `Entraram ${fmt(inn)}${vh?` (${fmt(vh)} voltaram da homologação)`:''}, saíram ${fmt(out)}${det?` (${det})`:''}.`;
  } else {
    t2 = 'Para ver o que entrou e o que saiu, restaure o HTML da coleta anterior no Passo 0 antes de processar.';
  }
  v.innerHTML = `<div class="vnum"><div class="num">${fmt(total)}</div><div class="vlab">em andamento</div></div>`+
    `<div><div class="t1 ${cls(delta)}">${t1}</div><div class="t2">${t2}</div><div class="t2">${extraLinks(cur,null)}</div></div>`;
  wireLinks(v);
}

// ---------- Legado x Atuais ----------
function statBlock(n, d, sp){
  return `<div class="stat"><span class="big num">${fmt(n)}</span><span class="lab">em andamento</span></div>`+
...
function renderCards(){
  const recs = andamentoRecs(cur);
  const leg = recs.filter(isLeg), atu = recs.filter(r=>!isLeg(r));
  const h = comparable(histOf(cur)), L = h.length-1;
  const fl = flowOf(cur,'passivo'), fa = flowOf(cur,'corrente');
  ...
  const le = document.getElementById('legExtra'), ae = document.getElementById('atuExtra');
  le.innerHTML = extraLinks(cur,'passivo'); ae.innerHTML = extraLinks(cur,'corrente'); wireLinks(le); wireLinks(ae);
...
function renderChart(){
  ...
  document.getElementById('chartTitle').textContent = `Evolução semanal do backlog em andamento: ${labelOf(cur)}`;
  const box = document.getElementById('chart');
  const hasSplit = h.some(x=>x.leg!==null && x.nova), hasOld = h.some(x=>!x.nova);
  document.getElementById('chartLegend').innerHTML =
    ...
    (hasOld ? '<span><span class="sw" style="background:var(--total)"></span>Regra antiga (incluía homologação)</span>' : '');
  document.getElementById('chartNote').textContent = hasOld
    ? 'As colunas cinza foram gravadas pela regra antiga, que contava as homologações junto, e não são comparáveis com as demais. Reprocessando as coletas antigas, todas passam para a regra nova.' : '';
  ...
  h.forEach((p,i)=>{
    ...
    if(p.nova && p.leg!==null){
      ...
      const ht = p.total*sc;
      g += `<rect x="${x}" y="${yb-ht}" width="${bw}" height="${ht}" rx="3" fill="var(--total)"><title>Regra antiga: ${p.total}</title></rect>`;
    }
    g += `<text x="${cx}" y="${yb-p.total*sc-8}" text-anchor="middle" font-size="13" font-weight="${last?700:500}" fill="${p.nova?'#1b2430':'#8793a4'}">${fmt(p.total)}</text>`;
  ...
  box.innerHTML = `<svg viewBox="0 0 ${W} ${H}" width="100%" style="min-width:640px" role="img" aria-label="Backlog em andamento por semana, ${labelOf(cur)}">${g}</svg>`;
}

// ---------- Aging ----------
const AGING = [['até 90 dias',0,90],['91 a 180',91,180],['181 a 365',181,365],['1 a 2 anos',366,730],['mais de 2 anos',731,1e9]];
const AGING_COR = ['#8fb3e8','#6f9bdc','#c9a15c','#c47a3a','#b3261e'];
function renderAging(){
  const recs = andamentoRecs(cur);
  document.getElementById('agTitle').textContent = `Aging do backlog em andamento: ${labelOf(cur)}`;
  const b = AGING.map(([lab,lo,hi])=>({lab,lo,hi,rs:recs.filter(r=>(r.diasAberto??0)>=lo && (r.diasAberto??0)<=hi)}));
  const mx = Math.max(1,...b.map(x=>x.rs.length)), tot = recs.length||1;
  const el = document.getElementById('aging');
  el.innerHTML = b.map((x,i)=>{
    const leg = x.rs.filter(isLeg).length;
    return `<button type="button" class="agcol" data-i="${i}" aria-label="Abrir ${x.rs.length} demandas com ${x.lab}">`+
      `<span class="agn">${fmt(x.rs.length)}</span>`+
      `<span class="agbar" style="height:${Math.max(4, x.rs.length/mx*150)}px;background:${AGING_COR[i]}"></span>`+
      `<span class="aglab">${x.lab}</span><span class="agpct">${Math.round(x.rs.length/tot*100)}% · legado ${fmt(leg)}</span></button>`;
  }).join('');
  el.querySelectorAll('.agcol').forEach(c=>c.onclick=()=>{
    const x = b[+c.dataset.i];
    openList(`Aging ${x.lab}: ${labelOf(cur)}`, x.rs, `aging_${cur}_${x.lo}`);
  });
  const o2 = b[4].rs.length;
  document.getElementById('agNote').textContent = o2
    ? `${fmt(o2)} demanda${o2===1?'':'s'} em andamento há mais de 2 anos. Clique numa coluna para ver as demandas.`
    : 'Clique numa coluna para ver as demandas.';
}
...
const GRUPOS = [['andamento', ANDAMENTO], ['homologacao', HOMOLOG], ['suspensa', ['Suspensa']]];
function renderStatus(){
  ...
  const row = st => { const rs = recs.filter(r=>r.status===st);
    return {st, leg:rs.filter(isLeg).length, atu:rs.filter(r=>!isLeg(r)).length, n:rs.length, d: prev ? rs.length-(prev[st]||0) : null}; };
  const all = status_order.map(row), mx = Math.max(1, ...all.map(r=>r.n));
  const bar = r => r.st==='Suspensa'
  ...
  let html='';
  GRUPOS.forEach(([g, sts])=>{
    const rows = sts.map(row), n = rows.reduce((a,r)=>a+r.n,0);
    const d = prev ? rows.reduce((a,r)=>a+(r.d||0),0) : null;
    const sub = g==='andamento' ? 'backlog aberto' : (g==='homologacao' ? 'entregues, com o cliente' : 'fora do backlog');
    html += `<button type="button" class="stgrp" data-g="${g}"><span class="nm">${GRUPO_NOME[g]}<small>${sub}</small></span><span></span><span class="n">${fmt(n)}</span><span class="dl ${g==='andamento'?cls(d):''}">${fmtD(d)}</span></button>`;
    if(g!=='suspensa') rows.forEach(r=>{
      html += `<button type="button" class="strow" data-st="${esc(r.st)}"><span class="nm">${r.st}</span>${bar(r)}<span class="n">${fmt(r.n)}</span><span class="dl ${g==='andamento'?cls(r.d):''}">${fmtD(r.d)}</span></button>`;
    });
  });
  const sb = document.getElementById('stBars'); sb.innerHTML = html;
  sb.querySelectorAll('.strow').forEach(b=>b.onclick=()=>openStatus(cur, b.dataset.st));
  sb.querySelectorAll('.stgrp').forEach(b=>b.onclick=()=>openGroup(cur, b.dataset.g, null));
  renderMatrix();
}
function renderMatrix(){
  const cols = [...NUCS, ALL];
  const count = (k, st) => ALL_RECORDS.filter(r=>inScope(r,k) && r.status===st).length;
  const countG = (k, g) => ALL_RECORDS.filter(r=>inScope(r,k) && grupoOf(r)===g).length;
  ...
  const btn = (v, attrs) => v ? `<button type="button" class="cellbtn" ${attrs}>${fmt(v)}</button>` : '<span class="zero">0</span>';
  const cell = (c, st) => {
    const v = count(c, st), bg = c!==ALL ? `background:${heat(v)};` : '';
    return `<td class="${c===ALL?'h':''} ${sel(c)?'colsel':''}" style="${bg}${sel(c)?'--c:'+colorOf(c):''}">${btn(v,`data-c="${c}" data-st="${esc(st)}"`)}</td>`;
  };
  const gcell = (c, g) => `<td class="${sel(c)?'colsel':''}" style="${sel(c)?'--c:'+colorOf(c):''}">${btn(countG(c,g),`data-c="${c}" data-g="${g}"`)}</td>`;
  let m = `<thead><tr><th>Status</th>${cols.map(c=>`<th${selAttr(c)}>${c}</th>`).join('')}</tr></thead><tbody>`;
  GRUPOS.forEach(([g, sts])=>{
    m += `<tr class="grp g-${g}"><td>${GRUPO_NOME[g]}${g==='andamento'?' <small>(backlog aberto)</small>':''}</td>${cols.map(c=>gcell(c,g)).join('')}</tr>`;
    if(g!=='suspensa') sts.forEach(st=>{ m += `<tr><td class="ind">${st}</td>${cols.map(c=>cell(c,st)).join('')}</tr>`; });
  });
  m += '</tbody>';
  const t = document.getElementById('mtx'); t.innerHTML = m;
  t.querySelectorAll('.cellbtn').forEach(b=>b.onclick=()=> b.dataset.g ? openGroup(b.dataset.c, b.dataset.g, null) : openStatus(b.dataset.c, b.dataset.st));
}

// ---------- Lista de demandas (diálogo) ----------
let dlgRows = [], dlgTag = '';
function openList(titulo, rows, tag){
  dlgRows = rows.slice().sort((a,b)=>(b.diasAberto||0)-(a.diasAberto||0));
  dlgTag = tag;
  document.getElementById('dlgTitle').textContent = titulo;
  ...
  document.getElementById('dlgBody').innerHTML = dlgRows.map(rowHTML).join('');
  const d = document.getElementById('dlg');
  if(d.showModal) d.showModal(); else d.setAttribute('open','');
}
const openStatus = (k, st) => openList(`${st}: ${labelOf(k)}`, ALL_RECORDS.filter(r=>inScope(r,k) && r.status===st), `${k}_${st}`);
function openGroup(k, g, bloco){
  const rows = ALL_RECORDS.filter(r=>inScope(r,k) && grupoOf(r)===g && (!bloco || (bloco==='passivo')===isLeg(r)));
  const bl = bloco ? (bloco==='passivo' ? ' (legado)' : ` (${ANO})`) : '';
  openList(`${GRUPO_NOME[g]}${bl}: ${labelOf(k)}`, rows, `${k}_${g}${bloco?'_'+bloco:''}`);
}

// ---------- Linha de demanda (mesma na listagem e no detalhe) ----------
const GESTOR_MAX = 15;
function rowHTML(r){
  ...
  const g = r.gestor || '';
  const gCurto = g.length > GESTOR_MAX ? g.slice(0, GESTOR_MAX).trimEnd() + '…' : g;
  return `
    <tr>
      ...
      <td class="mono">${r.esfFat ? fmt1(r.esfFat) : '—'}</td>
    </tr>`;
}
...
function populateTableFilters(){
  ...
  document.getElementById('filterStatus').innerHTML = '<option value="">Todos os status</option>'
    + `<optgroup label="Em andamento"><option value="g:andamento">Todos em andamento</option>${ANDAMENTO.map(s=>`<option value="${s}">${s}</option>`).join('')}</optgroup>`
    + `<optgroup label="Em homologação"><option value="g:homologacao">Todos em homologação</option>${HOMOLOG.map(s=>`<option value="${s}">${s}</option>`).join('')}</optgroup>`
...
function getTableRows(baseRecs){
  ...
  let rows = baseRecs.filter(r=>{
    ...
    if(st && (st.startsWith('g:') ? grupoOf(r)!==st.slice(2) : r.status!==st)) return false;
...
function renderTable(){
  ...
  document.getElementById('tableBody').innerHTML = rows.map(rowHTML).join('');
...
function exportRowsCSV(rows, sufixo){
  const header = ['ID','NGDS','Data criação','Título','Núcleo','Projeto','Status','Status real','Gerência (Prodam)','Gestor','Dias aberto','Prazo','Esf. estimado','Esf. realizado','Suspensa','Motivo suspensão','Início suspensão','Esf. faturado'];
  ...
  rows.forEach(r=>{
    lines.push([
      ...
      r.esfEst ?? '', r.esfReal ?? '', r.suspensa ? 'Sim' : 'Não', r.motivoSuspensao || '', r.dtInicioSuspensao || '', r.esfFat ?? ''
    ].map(csvEscape).join(';'));
  });
...
function renderHeader(){
  ...
  document.getElementById('metaTotalBacklog').textContent = fmt(ALL_RECORDS.filter(r=>grupoOf(r)==='andamento').length);
}
function renderAll(){
  renderFilter();
  renderVerdict();
