...
function exportTableCSV(){
  ...
  const header = ['ID','Data criação','Título','Núcleo','Projeto','Status','Gerência (Prodam)','Gestor','Dias aberto','Prazo','Esf. estimado','Esf. realizado'];
  const lines = [header.map(csvEscape).join(';')];
  rows.forEach(r=>{
    lines.push([
      r.id, r.dataCriacao || '', r.titulo, r.nucleoNegocio, r.sigla, r.status,
