...
.status-evol-spark{width:100%;height:40px;margin-top:8px;display:block;}
...
<div class="wrap">
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">03 · EVOLUÇÃO POR STATUS</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">04 · DEMANDAS SEM PRAZO</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">05 · TEMPO EM ABERTO</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">06 · ESFORÇO</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">07 · DETALHAMENTO</div>
...
function sparklinePoints(rows, w, h){
...
function renderStatusEvolution(){
  ...
  panel.innerHTML = '<div class="status-evol-grid">' + status_order.map(status=>{
    ...
    const pts = sparklinePoints(rows, W, H);
    ...
    const rangeTxt = rows.length>2
      ...
      : `${fmtDateBR(rows[0].data_snapshot)} → ${fmtDateBR(rows[rows.length-1].data_snapshot)}`;
    return `<div class="status-evol-card${susp?' suspensa':''}">
      <div class="status-evol-label" title="${status}">${status}</div>
  ...
  const lastDate = [...distinctDates].sort().pop();
  noteEl.textContent = `Última coleta: ${lastDate.split('-').reverse().join('/')}${nuc ? ' — núcleo '+nuc : ' — GDS-1 (todos os núcleos)'}.`;
}
