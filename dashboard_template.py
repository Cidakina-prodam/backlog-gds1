...
.mov-fill{height:8px;border-radius:4px;background:var(--accent);}
...
function renderVazao(){
  ...
  const cells = [
    {label:'Entraram', value:fmt(entradas), sub:'demandas criadas'},
    {label:'Concluídas', value:fmt(concluidas), sub:'entrega', cls:'', color:'var(--teal)'},
    {label:'Canceladas', value:fmt(canceladas), sub:'descarte'},
    {label:'Suspensas', value:fmt(suspensoes), sub:'congeladas', color:'var(--amber)'},
  ...
  document.getElementById('vazaoStrip').innerHTML = cells.map(c=>`
    <div class="vazao-cell${c.saldoCls?' saldo':''}">
