...
<div class="wrap">
  ...
  <section class="block">
    ...
    <div class="section-filter-bar">
      ...
      <select id="blocoNucleoSelect"></select>
      <span class="fb-label">Projeto</span>
      <select id="blocoProjetoSelect"></select>
    </div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">03 · PIPELINE</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">04 · EVOLUÇÃO POR STATUS</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">05 · DEMANDAS SEM PRAZO</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">06 · TEMPO EM ABERTO</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">07 · ESFORÇO</div>
  ...
  <section class="block">
    <div class="block-head">
      <div>
        <div class="block-num">08 · DETALHAMENTO</div>
...
function blocoRecsFiltrados(){
  const nuc = document.getElementById('blocoNucleoSelect').value;
  const proj = document.getElementById('blocoProjetoSelect').value;
...
function refreshBlocoProjetoOptions(){
  const sel = document.getElementById('blocoProjetoSelect');
  const nucleo = document.getElementById('blocoNucleoSelect').value;
  const atual = sel.value;
  const options = nucleo ? (DATA.projetoPorNucleo[nucleo]||[]) : DATA.projetos;
  sel.innerHTML = '<option value="">Todos os projetos</option>' + options.map(p=>`<option value="${p}">${p}</option>`).join('');
  if(options.includes(atual)) sel.value = atual;
}

function renderBlocos(){
  const recs = blocoRecsFiltrados();
  const defs = [
...
function initFilters(){
  document.addEventListener('click', function(e){
    ...
    if(s){ drillState.status = s.dataset.status; renderBlocos(); return; }
...
function populateSectionNucleoFilters(){
  ...
  const selBlocoNuc = document.getElementById('blocoNucleoSelect');
  selBlocoNuc.innerHTML = opts;
  refreshBlocoProjetoOptions();
  selBlocoNuc.addEventListener('change', ()=>{
    refreshBlocoProjetoOptions();
    document.getElementById('blocoProjetoSelect').value='';
    drillState = {bloco:null, status:null};
    renderBlocos();
  });
  document.getElementById('blocoProjetoSelect').addEventListener('change', ()=>{
    drillState = {bloco:null, status:null};
    renderBlocos();
  });
...
def build_dashboard_html(dataset: dict, history_rows: list, weekly_flow=None, status_history_rows: list = None) -> str:
    # weekly_flow ficou obsoleto com a remoção da seção de Fluxo Semanal; o
    # parâmetro continua na assinatura só para não quebrar chamadas antigas.
    data = dict(dataset)
