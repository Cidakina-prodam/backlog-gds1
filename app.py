...
if "status_history_rows" not in st.session_state:
    st.session_state.status_history_rows = []  # histórico por (núcleo, status) — evolução por status
...
with st.sidebar:
    ...
    if html_restore is not None:
        if st.button("📄 Extrair histórico desse HTML", use_container_width=True):
            hist_rows, mapping, records, snap_date, status_hist_rows, avisos_html = dp.extract_state_from_html(html_restore.getvalue())
            for a in avisos_html:
                st.warning(a)
            if hist_rows:
                st.session_state.history_rows = dp.append_history(st.session_state.history_rows, hist_rows)
            if status_hist_rows:
                st.session_state.status_history_rows = dp.append_status_history(st.session_state.status_history_rows, status_hist_rows)
            if mapping:
            ...
            if hist_rows or mapping or records or status_hist_rows:
    ...
    def make_backup_zip():
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            ...
            z.writestr("historico_status.csv", dp.status_history_to_csv_bytes(st.session_state.status_history_rows))
        ...
        if st.button("♻️ Restaurar agora", use_container_width=True):
            try:
                with zipfile.ZipFile(io.BytesIO(restore_file.getvalue())) as z:
                    ...
                    # historico_status.csv só existe em backups feitos depois dessa
                    # funcionalidade — backups antigos seguem restaurando o resto normalmente.
                    try:
                        status_hist_bytes = z.read("historico_status.csv")
                        st.session_state.status_history_rows = dp.status_history_from_csv_bytes(status_hist_bytes)
                    except KeyError:
                        st.info("Esse backup é anterior ao histórico por status — nada pra restaurar nesse ponto.")
...
if processar and uploaded:
    ...
    new_status_hist = dp.build_status_history_rows(dataset['records'], snapshot_date.isoformat())
    st.session_state.status_history_rows = dp.append_status_history(st.session_state.status_history_rows, new_status_hist)
...
if st.session_state.dataset is None:
    ...
    andamento = [r for r in dataset['records'] if dp.grupo_de(r) == 'andamento']
    n = len(andamento)
    sem_prazo = sum(1 for r in andamento if r['semPrazo'])
    c1, c2, c3 = st.columns(3)
    c1.metric("Backlog aberto (em andamento)", n)
    ...
    html = db.build_dashboard_html(dataset, st.session_state.history_rows, st.session_state.weekly_flow, st.session_state.status_history_rows)
    st.download_button(
        "📤 Gerar HTML para distribuição",
        data=html.encode("utf-8"),
        file_name=f"backlog_gds1_{dataset['snapshotDate']}.html",  # data da coleta, não a de hoje
        mime="text/html",
        type="primary",
    )
